...
from arcrest import MapService as ArcMapService, ImageService as ArcImageService, FeatureService as ArcFeatureService
...
class ArcMapServiceHandler(base.ServiceHandlerBase):
    ...
    def get_harvester_configuration_options(self):
        return {"harvest_map_services": True, "harvest_image_services": False, "harvest_feature_services": False}
...
class ArcFeatureServiceHandler(ArcMapServiceHandler):
    ...
    @property
    def parsed_service(self):
        return ArcFeatureService(self.url)
    ...
    def get_harvester_configuration_options(self):
        return {"harvest_map_services": False, "harvest_image_services": False, "harvest_feature_services": True}

    def _parse_datasets(self, layers):
        feature_datasets = []
        for lyr in layers:
            feature_datasets.append(self._dataset_meta(lyr))
            feature_datasets.extend(self._parse_datasets(lyr.subLayers))
    ...
    def _get_indexed_dataset_fields(self, dataset_meta):
        ...
        return {
            ...
            "bbox_polygon": BBOXHelper.from_xy([bbox[0], bbox[2], bbox[1], bbox[3]]).as_polygon(),
            "srid": srs,
            "keywords": ["ESRI", "ArcGIS REST FeatureServer", dataset_meta.title],
        }
...
class ArcImageServiceHandler(ArcMapServiceHandler):
    ...
    def get_harvester_configuration_options(self):
        return {"harvest_map_services": False, "harvest_image_services": True, "harvest_feature_services": False}
